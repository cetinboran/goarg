...
+ 30.08.2023
    + The goarg start function now returns two distinct values. The first one is a args structure containing the arguments, and the second one is an error structure that includes error messages added by you corresponding to the input values. With this update, your code will have become more comprehensible and manageable.
    ...
    + The `GetErrors` method takes one arguments. The first one is the number of the error you want to display on the screen. This number corresponds to the order of the Error string array that you added using `AddError`. If you want to see the first error you added, you'll write 1.
